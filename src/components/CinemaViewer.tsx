...
export default function CinemaViewer({ photos, albumName }: CinemaViewerProps) {
  ...
  // Touch/swipe support (desktop cinema only — mobile uses native scroll)
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className="md:hidden min-h-screen bg-black">
        {/* Sticky header */}
        <div className="sticky top-0 z-40 bg-black/90 backdrop-blur-sm border-b border-white/5">
          <div className="flex items-center justify-between px-4 h-14">
            <Link
              href="/work"
              className="text-white/40 hover:text-white/70 transition-colors text-xs tracking-[0.2em] uppercase"
            >
              &larr; Work
            </Link>
            {albumName && (
              <h2
                className="text-white text-sm font-bold tracking-normal truncate max-w-[60%] text-center"
      ...
      {/* ===== DESKTOP: Cinema viewer (unchanged) ===== */}
