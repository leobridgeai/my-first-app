...
function SortablePhotoRow({
  ...
  album: Album;
  isEditing: boolean;
  photoForm: { title: string; description: string; sortOrder: number; isFeatured: boolean };
  setPhotoForm: (form: { title: string; description: string; sortOrder: number; isFeatured: boolean }) => void;
  onStartEdit: () => void;
  onSaveEdit: () => void;
  onCancelEdit: () => void;
  onSetCover: () => void;
          ...
          <div className="flex gap-1 flex-shrink-0">
            ...
            >
              <svg
                ...
                className="w-4 h-4"
                fill={
                  album.coverPhotoId === photo.id ? "currentColor" : "none"
                }
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1.5}
...
export default function AlbumDetailPage() {
  ...
  async function handlePhotoDragEnd(event: DragEndEvent) {
    ...
    setAlbum({
      ...album,
      photos: reordered.map((photo) => ({ photo })),
    });

    setSavingOrder(true);
    await fetch(`/api/albums/${id}/photos/reorder`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
            ...
            <div className="space-y-3">
              {photos.map((photo) => (
                <SortablePhotoRow
                  ...
                  onStartEdit={() => startEditingPhoto(photo)}
                  ...
                  onRemove={() => removePhotoFromAlbum(photo.id)}
